...
def create_model(config: DictConfig, vocab_size: int) -> Ares:
    ...
    print("INTERLEAVING: ", model_cfg.moe_interleaved)
...
def distributed_rendezvous(enabled: bool, tag: str):
    """Synchronize all ranks at critical startup and validation boundaries."""
    if not enabled:
        return
...
def validation_epoch(
    ...
    for difficulty_idx in range(len(config.masking.mlm_probability)):
        ...
        distributed_rendezvous(
            is_distributed,
            f"val_start_difficulty_{difficulty_idx}",
        )
        ...
        for batch_idx, batch in enumerate(pbar, start=1):
            ...
            if (
                ...
                distributed_rendezvous(
                    is_distributed,
                    f"val_progress_difficulty_{difficulty_idx}_batch_{batch_idx}",
                )
        ...
        wandb_log_on_master(
            {
                f"val@{difficulty_idx}/loss": source.total_loss,
                f"val@{difficulty_idx}/perplexity": source.total_perplexity,
                f"val@{difficulty_idx}/accuracy": source.total_accuracy,
                f"val@{difficulty_idx}/epoch": epoch,
            },
            step=step_to_log,
        )
        distributed_rendezvous(
            is_distributed,
            f"val_end_difficulty_{difficulty_idx}",
        )
...
# ── Entrypoint ────────────────────────────────────────────────────────────────
# Select a config variant with:  python train_soft_moe.py --config-name=<name>
# e.g. config_soft_moe_v6e_l2, config_soft_moe_v6e_interleaved, config_soft_moe_v5e
@hydra.main(
    config_path="config",
    config_name="config_soft_moe_v6e",
    version_base=None,
)
