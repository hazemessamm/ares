...
def convert_to_jsonable(obj: Any) -> Any:
    ...
    if isinstance(obj, dict):
        return {
            str(key): convert_to_jsonable(value) for key, value in obj.items()
        }
        ...
        worst_layers.append(
            {
                "layer": layer_name,
                "rating": rating,
                "combine_mass_entropy_norm": stats[
                    "combine_mass_entropy_norm"
                ],
                "top1_entropy_norm": stats["top1_entropy_norm"],
                "top1_max_share": stats["top1_max_share"],
                "slot_top_weight_p95": stats["slot_top_weight_p95"],
...
def analyze_checkpoint(
    ...
    dataloader = DataLoader(
        dataset, batch_size=batch_size, collate_fn=Collator(tokenizer)
    )
    model.eval()
    ...
    for batch in tqdm(dataloader, desc="Analyzing Soft-MoE"):
        ...
        for name, module in model.named_modules():
            ...
            valid_dispatch = dispatch[
                valid_mask[:, :, None, None].expand_as(dispatch)
            ]
            ...
            layer_dispatch_stats[name]["slot_top_weights"].append(
                slot_top_weights
            )
            layer_dispatch_stats[name]["token_total_mass"].append(
                token_total_mass
            )
            layer_dispatch_stats[name]["seq_softmax_sum_min"].append(
                float(per_slot_sums.min())
            )
            layer_dispatch_stats[name]["seq_softmax_sum_max"].append(
                float(per_slot_sums.max())
            )
            ...
            layer_dispatch_token_mass.setdefault(name, []).append(
                token_total_mass
            )
    ...
    for name in sorted(layer_expert_mass.keys()):
        ...
        expert_balance[name] = {
            ...
            "combine_mass_top5_shares": [
                float(x) for x in sorted(mass_probs, reverse=True)[:5]
            ],
            ...
            "token_expert_entropy_p05_norm": float(
                np.quantile(token_entropy, 0.05)
            ),
            "token_expert_entropy_p50_norm": float(
                np.quantile(token_entropy, 0.50)
            ),
            "token_expert_entropy_p95_norm": float(
                np.quantile(token_entropy, 0.95)
            ),
            "slot_top_weight_mean": float(slot_top_weights.mean()),
            "slot_top_weight_p95": float(np.quantile(slot_top_weights, 0.95)),
            "slot_top_weight_max": float(slot_top_weights.max()),
...
def print_human_summary(results: Dict[str, Any]) -> None:
    ...
    print(
        ...
        "mean={mean:.3e} p95={p95:.3e} p99={p99:.3e} max={max:.3e}".format(
            **dispatch
        )
    )
    ...
    print(
        f"healthy={counts['healthy']} warning={counts['warning']} bad={counts['bad']}" # noqa
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Soft-MoE checkpoint diagnostics"
    )
    parser.add_argument(
        "--checkpoint",
    ...
    parser.add_argument(
        "--max-examples", type=int, default=50, help="Number of sequences"
    )
