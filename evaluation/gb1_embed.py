...
def main(cfg: DictConfig):
    print("Running gb1 evaluation on model: ", cfg.model.name, "with checkpoint: ", getattr(cfg, cfg.model.name).checkpoint, "and seed: ", cfg.training.seed)
    ...
    output_dir = os.path.join(model_config.output_dir, "gb1", f"seed_{cfg.training.seed}")
    ...
    train_embed_output_dir = os.path.join(model_config.output_dir, "gb1", "embeddings", "train")
    validation_embed_output_dir = os.path.join(model_config.output_dir, "gb1", "embeddings", "validation")
    test_embed_output_dir = os.path.join(model_config.output_dir, "gb1", "embeddings", "test")
    ...
    embed_train_dataset = dataset_module.GB1Embeddings(train_embed_output_dir, "train")
    embed_validation_dataset = dataset_module.GB1Embeddings(validation_embed_output_dir, "validation")
    embed_test_dataset = dataset_module.GB1Embeddings(test_embed_output_dir, "test")
