...
class SteeringConfig:
    ...
    layer_expert_alphas: Dict[str, Dict[int, float]] = field(
        default_factory=dict
    )

    def scales_for_layer(
...
def _make_steered_forward(
    ...
    def steered_forward(
        ...
        s = scales.to(
            device=combine_weights.device, dtype=combine_weights.dtype
        )
        s_expanded = s.view(1, 1, E, 1)  # broadcast over batch, seq, slots

        scaled = combine_weights * s_expanded
        denom = scaled.sum(dim=(2, 3), keepdim=True).clamp_min(
        ...
        if position_mask is not None:
            ...
            combine_weights = torch.where(
                pm_expanded, steered, combine_weights
            )
        else:
...
def property_prob_mass(
    ...
    group_mask = torch.zeros(
        vocab_size, dtype=torch.bool, device=logits_at_masked.device
    )
...
def evaluate_steering_shift(
    ...
    with torch.no_grad():
        ...
        # Steered forward
        with apply_steering(
            model, steering_config, position_mask=masked_positions
        ):
    ...
    logits_steer_m = logits_steer.reshape(-1, logits_steer.shape[-1])[
        flat_mask
    ]

    baseline_mass = property_prob_mass(
        logits_base_m, id_to_aa, target_property, ignore_ids
    )
    steered_mass = property_prob_mass(
        logits_steer_m, id_to_aa, target_property, ignore_ids
    )
